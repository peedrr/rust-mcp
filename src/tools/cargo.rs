...
pub async fn analyze_manifest_impl(args: Value, analyzer: &mut RustAnalyzerClient) -> Result<ToolResult> {
    let manifest_path = args.get("manifest_path")
        .and_then(|v| v.as_str())
...
pub async fn run_cargo_check_impl(args: Value, analyzer: &mut RustAnalyzerClient) -> Result<ToolResult> {
