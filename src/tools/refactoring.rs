...
pub async fn extract_function_impl(args: Value, analyzer: &mut RustAnalyzerClient) -> Result<ToolResult> {
    let file_path = args.get("file_path")
        .and_then(|v| v.as_str())
        .ok_or_else(|| anyhow::anyhow!("Missing file_path parameter"))?;
    ...
    let start_character = args.get("start_character")
        .and_then(|v| v.as_u64())
    ...
    let end_character = args.get("end_character")
        .and_then(|v| v.as_u64())
    ...
    let result = analyzer.extract_function(
        file_path, 
        start_line as u32, 
...
pub async fn inline_function_impl(args: Value, analyzer: &mut RustAnalyzerClient) -> Result<ToolResult> {
    let file_path = args.get("file_path")
        .and_then(|v| v.as_str())
        .ok_or_else(|| anyhow::anyhow!("Missing file_path parameter"))?;
    let line = args.get("line")
    ...
    let result = analyzer.inline_function(file_path, line as u32, character as u32).await?;
...
pub async fn change_signature_impl(args: Value, analyzer: &mut RustAnalyzerClient) -> Result<ToolResult> {
    let file_path = args.get("file_path")
        .and_then(|v| v.as_str())
        .ok_or_else(|| anyhow::anyhow!("Missing file_path parameter"))?;
    let line = args.get("line")
    ...
    let character = args.get("character")
        .and_then(|v| v.as_u64())
        .ok_or_else(|| anyhow::anyhow!("Missing character parameter"))?;
    let new_signature = args.get("new_signature")
        .and_then(|v| v.as_str())
        .ok_or_else(|| anyhow::anyhow!("Missing new_signature parameter"))?;

    let result = analyzer.change_signature(file_path, line as u32, character as u32, new_signature).await?;
...
pub async fn organize_imports_impl(args: Value, analyzer: &mut RustAnalyzerClient) -> Result<ToolResult> {
    let file_path = args.get("file_path")
        .and_then(|v| v.as_str())
        .ok_or_else(|| anyhow::anyhow!("Missing file_path parameter"))?;
