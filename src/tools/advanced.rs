...
pub async fn get_type_hierarchy_impl(args: Value, analyzer: &mut RustAnalyzerClient) -> Result<ToolResult> {
    ...
    let result = analyzer.get_type_hierarchy(file_path, line as u32, character as u32).await?;
...
pub async fn suggest_dependencies_impl(args: Value, analyzer: &mut RustAnalyzerClient) -> Result<ToolResult> {
...
pub async fn create_module_impl(args: Value, analyzer: &mut RustAnalyzerClient) -> Result<ToolResult> {
    let module_name = args.get("module_name")
        .and_then(|v| v.as_str())
        .ok_or_else(|| anyhow::anyhow!("Missing module_name parameter"))?;
    let module_path = args.get("module_path")
        .and_then(|v| v.as_str())
        .ok_or_else(|| anyhow::anyhow!("Missing module_path parameter"))?;
    let is_public = args.get("is_public")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);

    let result = analyzer.create_module(module_name, module_path, is_public).await?;
...
pub async fn move_items_impl(args: Value, analyzer: &mut RustAnalyzerClient) -> Result<ToolResult> {
    ...
    let target_file = args.get("target_file")
        .and_then(|v| v.as_str())
        .ok_or_else(|| anyhow::anyhow!("Missing target_file parameter"))?;
    let item_names_value = args.get("item_names")
    ...
    let result = analyzer.move_items(source_file, target_file, &item_names).await?;
